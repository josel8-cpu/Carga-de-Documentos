...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.cambiarEstatus = function(id, nuevoEstatus) {
        ...
        if (pago) {
            ...
            if (!window._actualizandoEstatus) {
                const estatusTexto = { 'pendiente': 'Pendiente', 'aplicado': 'Aplicado', 'cancelado': 'Cancelado', 'revision': 'En Revisión' };
    ...
    function inicializarRegistroGarantia() {
        ...
        function actualizarSeccionDocumento() {
            ...
            if (facturaCard) facturaCard.hidden = (tipo !== 'FACTURA');
            if (testimonioNotarialCard) testimonioNotarialCard.hidden = (tipo !== 'TESTIMONIO NOTARIAL');
            if (actaPosesionCard) actaPosesionCard.hidden = (tipo !== 'ACTA DE POSESIÓN');
            if (reciboSimpleCard) reciboSimpleCard.hidden = (tipo !== 'RECIBO SIMPLE');
            if (constanciaPosesionCard) constanciaPosesionCard.hidden = (tipo !== 'CONSTANCIA DE POSESIÓN');
        }
    ...
    /* ---------------------------------------------------------
       15. AUTO-COMPLETADO DE OBSERVACIONES EN TIEMPO REAL
    --------------------------------------------------------- */
    ...
    const COMPLEMENTARIOS = [
        'solicitud de crédito firmada', 'factura', 'carta factura', 'aclaración de firmas',
        'acta de supervisión', 'reporte fotográfico y tratamiento',
        'autorización entrega de vehículo y resguardo de llave', 'carta consentimiento',
        'póliza seguro', 'póliza cotización', 'endoso jurídico', 'constancia recepción de caratula',
        'carta aceptación', 'carta compromiso', 'formato de sustitución garantia',
        'garantia adicional', 'GPS', 'datos complementarios (otros)', 'validación guardavalores'
    ];
    const DOCUMENTOS_ACREDITADO = [
        ...
        'Autorización de Consulta de Buró', 'Carta de Excepción a la Norma', 'Enganche',
        'Garantia Liquida', 'Comisión por Apertura', 'Co Financiamiento',
        'Complemento de Enganche', 'Garantia Liquida Enganche Diferido',
        'Factura', 'Carta Factura', 'Permiso', 'Enganche GPS'
    ];
    const DOCUMENTOS_AVAL = [
        'Identificacion Oficial Aval', 'CURP Aval', 'RFC Aval', 'Acta nacimiento aval',
        'Comprobante domicilio aval', 'Comprobante de Ingresos Aval',
        'Garantia Liquida Enganche Diferido', 'Factura Aval', 'Carta Factura Aval',
        'Permiso Aval', 'Enganche GPS Aval',
        'Comprobante Domicilio Alterno Aval Solidario', 'Arraigo Domiciliar Aval Solidario',
        'Fotos del Domicilio Aval Solidario', 'Fotos de la Actividad Aval Solidario',
        'Comprobante de Ingresos Aval Solidario', 'Otros Aval Solidario'
    ];
    const VIDEO_DESEMBOLSO = ['VIDEO'];
    const DOCUMENTOS_GARANTIA = [
        'Factura', 'Carta Factura', 'Carta de Aceptación de Endoso',
        'Garantía Adicional 1', 'Garantía Adicional 2', 'Pagaré de Distribuidor',
        'Cotización del Seguro', 'Póliza del Seguro', 'Comprobante de Pago del Seguro',
        'Solicitud de Instalación del GPS', 'Validación de Factura SAT',
        'Estado de Cuenta', 'Otros'
    ];

    let kitEstado = {};

    function inicializarKitEstado() {
        const todos = [...KIT_DOCUMENTOS, ...COMPLEMENTARIOS, ...DOCUMENTOS_ACREDITADO, ...DOCUMENTOS_AVAL, ...VIDEO_DESEMBOLSO, ...DOCUMENTOS_GARANTIA];
        todos.forEach(nom => {
            if (!kitEstado[nom]) {
    ...
    function renderKitTabla(tbodyId, listaDocs) {
        ...
        if (!tbody) return;
        tbody.innerHTML = '';
        listaDocs.forEach(nom => {
            const estado = kitEstado[nom] || { archivo: null, validado: false, comentario: '', historial: [] };
            const tr = document.createElement('tr');
            const fileId = `kitFile_${nom.replace(/\s/g, '_')}`;
            tr.innerHTML = `
    ...
    window.toggleValidarKit = function(nombre, checked) {
        ...
        // No re-renderizar toda la tabla, solo actualizar el switch ya está reflejado.
        // Si se quiere actualizar otros elementos visuales, se puede llamar a renderAllKitTables().
        // Pero para mantener la fluidez, no es necesario.
    };
    ...
    window.mostrarListaKit = function() {
        const lista = document.getElementById('kitListaContainer');
        const carga = document.getElementById('kitCargaContainer');
        if (lista) lista.style.display = 'block';
        if (carga) carga.style.display = 'none';
        document.querySelectorAll('.liberaciones-izquierda .btn-kit').forEach(btn => {
            btn.style.background = 'var(--bg-panel-2)';
            btn.style.borderColor = 'var(--border)';
        });
        const btnGen = document.querySelector('.liberaciones-izquierda .btn-kit:first-child');
        if (btnGen) {
    ...
    window.mostrarCargaKit = function() {
        ...
        const btnCarga = document.querySelector('.liberaciones-izquierda .btn-kit:last-child');
        if (btnCarga) {
    ...
    window.guardarCargaKit = function() {
        alert('Carga de documentos del Kit Legal guardada (simulación)');
    };

    // Corregido: oculta también 'solicitudLiberacion'
    window.mostrarLiberaciones = function() {
        ...
        document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
        const secciones = [
            'firmas', 'dictamen', 'dictamenQuash', 'historial',
            'tipoActa', 'generarActa', 'cargaDocumentos', 'aplicacionPagos',
            'registroGarantia', 'valuacion', 'factura', 'testimonioNotarial',
            'actaPosesion', 'reciboSimple', 'constanciaPosesion',
            'solicitudLiberacion'  // Oculta la vista de Liberación
        ];
        secciones.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.hidden = true;
        });
        const liberaciones = document.getElementById('liberaciones');
        if (liberaciones) liberaciones.hidden = false;
        const btnDoc = document.getElementById('btnDocumentosCliente');
        const btnCarat = document.getElementById('btnCaratulaComite');
    ...
    function actualizarConteos() {
        const conteos = { solicitado: 0, liberado: 0, observado: 0, porrevisar: 0 };
        SOLICITUD_DOCUMENTOS.forEach(nom => {
            const est = solicitudEstado[nom].estado;
            if (conteos[est] !== undefined) conteos[est]++;
        });
        document.getElementById('countSolicitado').textContent = conteos.solicitado;
        document.getElementById('countLiberado').textContent = conteos.liberado;
        document.getElementById('countObservado').textContent = conteos.observado;
        document.getElementById('countPorRevisar').textContent = conteos.porrevisar;
    }

    function renderSolicitud() {
        const tbody = document.getElementById('solicitudBody');
        if (!tbody) return;
        tbody.innerHTML = '';
        SOLICITUD_DOCUMENTOS.forEach(nom => {
            const estado = solicitudEstado[nom];
            const tr = document.createElement('tr');
            const fileId = `solFoto_${nom.replace(/\s/g, '_')}`;
            const opcionesObs = ['ilegible', 'error firmas', 'no cotejado', 'no valido', 'error en doctor'];
            let obsOptionsHtml = '';
            opcionesObs.forEach(obs => {
                const selected = estado.observaciones.includes(obs) ? 'selected' : '';
            ...
            tr.innerHTML = `
                ...
                <td>
                    <!-- Select múltiple con size="1" para que parezca un dropdown -->
    ...
    window.verSolicitudDocumento = function(nombre) {
        const doc = solicitudEstado[nombre];
    ...
    window.guardarSolicitud = function() {
        let resumen = 'Resumen de Solicitud de Liberación:\n\n';
        SOLICITUD_DOCUMENTOS.forEach(nom => {
            const doc = solicitudEstado[nom];
            resumen += `${nom}: Estado=${doc.estado}, Validado=${doc.validado ? 'Sí' : 'No'}, Obs=${doc.observaciones.join(', ') || 'Ninguna'}\n`;
        });
    ...
    window.mostrarSolicitudLiberacion = function() {
        ...
        const solicitud = document.getElementById('solicitudLiberacion');
